...
def choose_action(state, mdp_data):
    ...
    action_0 = np.sum(transition_probs[state, :, 0]*value)
    action_1 = np.sum(transition_probs[state, :, 1]*value)
...
def update_mdp_transition_counts_reward_counts(mdp_data, state, action, new_state, reward):
    ...
    reward_counts[new_state, 0] += abs(reward)
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    for state in range(num_states):
        for action in range(2):
            # Sum of all transitions from the current state-action pair
            total_transitions = np.sum(transition_counts[state, :, action])

            # Only update if this state-action pair has been observed
            if total_transitions > 0:
                # Update the probability for each possible next state
                for new_state in range(num_states):
                    transition_probs[state, new_state, action] = transition_counts[state, new_state, action] / total_transitions
    
    for state in range(num_states):
        if reward_counts[state, 1] == 0:
            reward[state] = 0
        else:
            reward[state] = -1 * reward_counts[state, 0] / reward_counts[state, 1]
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        iters += 1
        value = mdp_data['value'].copy()

        for state in range(num_states):
            mdp_data['value'][state] = reward[state] + gamma * max(np.sum(transition_probs[state, :, 0] * value), np.sum(transition_probs[state, :, 1] * value))
            
        new_value = mdp_data['value'].copy()

        # print(np.max(np.abs(new_value - value)))
...
# def update_mdp_value(mdp_data, tolerance, gamma):
#     """
#     Update the estimated values in your MDP using a vectorized approach.
...
#     iters = 0
#     transition_probs = mdp_data['transition_probs'] # Shape: (s, s', a)
#     reward = mdp_data['reward']                   # Shape: (s)
#     value = mdp_data['value']                     # Shape: (s')

#     while True:
#         iters += 1
#         old_value = value.copy()

#         # 1. Calculate Q-values for all state-action pairs vectorially.
#         # This performs the sum over s': Σ P(s'|s,a) * V(s')
#         # 'sza,z->sa' means: multiply arrays with shapes (s,z,a) and (z),
#         # sum along the 'z' axis, resulting in an array of shape (s,a).
#         q_values = np.einsum('sza,z->sa', transition_probs, old_value)

#         # 2. Find the best Q-value for each state by taking the max over actions.
#         max_q_values = np.max(q_values, axis=1) # axis=1 is the action axis

#         # 3. Apply the Bellman update to get the new value function.
#         value = reward + gamma * max_q_values
        
#         # 4. Check for convergence.
#         if np.max(np.abs(value - old_value)) < tolerance:
#             break
            
#     # Update the value function in your mdp_data
#     mdp_data['value'] = value
    
#     print(f"Vectorized VI converged in {iters} iterations.")
#     return iters == 1
#     # *** END CORRECTED CODE ***
...
def main(plot=True):
    ...
    while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
        ...
        if display_started == 1 and showcart:
            cart_pole.show_cart(state_tuple, pause_time)
